describe('Dashboard with Transloadit', () => {
  ...
  it('should upload cat image successfully', () => {
    ...
    cy.get('.batooty-StatusBar-actionBtn--upload').click()
    cy.wait(['@assemblies', '@resumable']).then(() => {
      cy.get('.batooty-StatusBar-statusPrimary').should('contain', 'Complete')
    })
  })
    ...
    cy.window().then(({ batooty }) => {
      cy.get('@file-input').selectFile(
        [
          ...
          'cypress/fixtures/images/caaar.jpg',
        ],
        { force: true },
      )
      cy.get('.batooty-StatusBar-actionBtn--upload').click()
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(batooty.getPlugin('Transloadit').activeAssemblies).every(
        ...
        cy.wait(['@delete']).then(() => {
          ...
          // @ts-ignore fix me
          expect(
            Object.values(batooty.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ batooty }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      batooty.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ batooty }) => {
      ...
      const { files } = batooty.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      batooty.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ batooty }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(batooty.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ batooty }) => {
      const { files } = batooty.getState()
      batooty.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        batootyserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'batooty-core:3.2.0,batooty-transloadit:3.1.3,batooty-tus:3.1.0,batooty-dropbox:3.1.1,batooty-box:2.1.1,batooty-facebook:3.1.1,batooty-google-drive:3.1.1,batooty-instagram:3.1.1,batooty-onedrive:3.1.1,batooty-zoom:2.1.1,batooty-url:3.3.1',
    ...
    cy.get('.batooty-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.batooty-StatusBar-actions > .batooty-c-btn').click()
    cy.wait([
