...
describe('Dashboard with XHR', () => {
  ...
  it('should return correct file name with URL plugin from remote image with Content-Disposition', () => {
    ...
    cy.get('.batooty-Url-input').type(
      'http://localhost:4678/file-with-content-disposition',
    )
    interceptCompanionUrlMetaRequest()
    cy.get('.batooty-Url-importButton').click()
    cy.wait('@url-meta').then(() => {
      cy.get('.batooty-Dashboard-Item-name').should('contain', fileName)
      cy.get('.batooty-Dashboard-Item-status').should('contain', '84 KB')
    })
  })

  it('should return correct file name with URL plugin from remote image without Content-Disposition', () => {
    cy.get('[data-cy="Url"]').click()
    cy.get('.batooty-Url-input').type('http://localhost:4678/file-no-headers')
    interceptCompanionUrlMetaRequest()
    cy.get('.batooty-Url-importButton').click()
    cy.wait('@url-meta').then(() => {
      cy.get('.batooty-Dashboard-Item-name').should('contain', 'file-no')
      cy.get('.batooty-Dashboard-Item-status').should('contain', '0')
    })
  })
  ...
  it('should return correct file name even when Companion doesnt supply it', () => {
    ...
    cy.get('.batooty-Url-importButton').click()
    cy.wait('@url-meta').then(() => {
      cy.get('.batooty-Dashboard-Item-name').should('contain', 'file-with')
      cy.get('.batooty-Dashboard-Item-status').should('contain', '123 B')
    })
  })
