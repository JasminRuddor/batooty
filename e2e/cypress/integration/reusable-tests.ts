...
export function runRemoteUrlImageUploadTest() {
  ...
  cy.get('.batooty-Url-input').type(
    'https://raw.githubusercontent.com/transloadit/batooty/main/e2e/cypress/fixtures/images/cat.jpg',
  )
...
export function runRemoteUnsplashUploadTest() {
  ...
  cy.get('.batooty-SearchProvider-input').type('book')
  cy.intercept({
    method: 'GET',
    url: 'http://localhost:3020/search/unsplash/list?q=book',
  }).as('unsplash-list')
  cy.get('.batooty-SearchProvider-searchButton').click()
  cy.wait('@unsplash-list')
  // Test that the author link is visible
  cy.get('.batooty-ProviderBrowserItem')
    .first()
    .within(() => {
      cy.root().click()
  ...
  cy.get('.batooty-c-btn-primary').click()
  cy.intercept({
    method: 'POST',
