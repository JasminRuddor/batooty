...
const packageNames = await fs.readdir(new URL('../packages/@batooty', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@batooty/core is automatically included',
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@batooty/${pgk}'`).join('\n')}
...
const prettyPath = (url) => url.toString().split('batooty', 2)[1]
