...
class MarkdownTextarea {
  ...
  setupbatooty = () => {
    this.batooty = new batooty({ autoProceed: true })
      .use(Transloadit, {
    ...
    this.batooty.on('complete', (result) => {
  ...
  uploadFiles = (files) => {
    const filesForbatooty = files.map(file => {
      return {
        data: file,
        type: file.type,
        name: file.name,
    ...
    this.batooty.addFiles(filesForbatooty)
  }
}
